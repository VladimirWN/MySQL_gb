...
SELECT *
FROM shops
WHERE EXISTS (SELECT * FROM cats WHERE shops.id = cats.shops_id AND cats.name = "Murzik");

-- Вывести магазины, в котором НЕ продаются коты “Мурзик” и “Zuza”
SELECT *
FROM shops
WHERE NOT EXISTS (SELECT * FROM cats WHERE shops.id = cats.shops_id AND (cats.name = "Murzik" OR cats.name = "Zuza"));
